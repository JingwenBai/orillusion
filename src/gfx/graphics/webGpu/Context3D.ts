...
export class Context3D extends CEventDispatcher {
     ...
     * The WebGPU adapter and device are shared across all engine instances;
     * only the canvas swap-chain is created per instance.
     * @param canvasConfig
     * @returns
     */
    async init(canvasConfig?: CanvasConfig): Promise<boolean> {
        ...
        if (!Context3D._sharedAdapter) {
            // request adapter — only once for all engine instances
            ...
            // request device — only once for all engine instances
        ...
        // Reuse the shared adapter and device
        ...
        // configure per-instance canvas swap-chain
...
/**
 * @internal
 * The currently active WebGPU context. Set by whichever Engine3D instance is
 * currently initialising or rendering. Since JavaScript is single-threaded,
 * only one engine renders at a time, making this safe for multi-instance use.
 */
export let webGPUContext: Context3D = null;

/**
 * @internal
 * Update the module-level webGPUContext pointer. Called by Engine3D on init
 * and at the start of every render frame so that all downstream code
 * automatically sees the active engine's context.
 */
