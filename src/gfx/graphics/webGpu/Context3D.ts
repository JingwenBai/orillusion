...
export class Context3D extends CEventDispatcher {
    ...
    // Per-instance canvas/presentation state
    ...
    // Shared GPU resources across all Engine3D instances (initialized on first init())
    ...
    /**
     * The currently-active Context3D (set by Engine3D before each render frame).
     * @internal
     */
    public static current: Context3D;
     ...
     * The GPU adapter and device are created only on the first call and reused
     * by all subsequent Context3D instances (multi-instance support).
     */
    async init(canvasConfig?: CanvasConfig): Promise<boolean> {
        ...
        // Initialize shared adapter and device only once across all instances
        ...
        // Per-instance canvas context setup
        ...
        // Become the current context if none is set yet
        if (!Context3D.current) {
            Context3D.current = this;
        }
...
/**
 * The currently-active WebGPU context.  Engine3D switches this before every
 * render frame so that all internal GPU code automatically targets the right
 * canvas swap-chain.
 * @internal
 */
...
/**
 * Switch the active WebGPU context.  Called by Engine3D at the start of each
 * render frame to ensure the right per-canvas context is in scope.
 * @internal
 */
export function setActiveContext(ctx: Context3D): void {
    webGPUContext = ctx;
    Context3D.current = ctx;
}
