...
export class Context3D extends CEventDispatcher {
    ...
    // Shared across all Context3D instances (one GPU device per page)
    ...
    // Per-instance canvas/context state
    ...
    // Accessors delegating to shared GPU device
    ...
    /**
     * Configure canvas by CanvasConfig. On first call also requests the shared GPU device.
     * Subsequent calls reuse the same device for additional canvases.
     * @param canvasConfig
     * @returns
     */
    async init(canvasConfig?: CanvasConfig): Promise<boolean> {
        ...
        // Initialize shared GPU device once
...
/**
 * The currently active WebGPU context. Swapped per-engine before each render frame.
 * @internal
 */
export let webGPUContext: Context3D = new Context3D();

/**
 * Set the active WebGPU context. Called by Engine3D before each render frame
 * so that all subsystem static helpers read the correct per-engine canvas context.
 * @internal
 */
export function setActiveGPUContext(ctx: Context3D) {
    webGPUContext = ctx;
}
