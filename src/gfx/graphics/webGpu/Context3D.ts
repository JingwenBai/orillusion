...
export class Context3D extends CEventDispatcher {
    ...
    // Shared GPU resources across all instances on the same page
    ...
    async init(canvasConfig?: CanvasConfig): Promise<boolean> {
        ...
        // Share the GPU adapter across all Context3D instances
        ...
        // Share the GPU device across all Context3D instances
        ...
        // Configure per-canvas swap chain
...
/**
 * @internal
 * Active WebGPU context — updated by Engine3D when switching between engine instances.
 */
export let webGPUContext: Context3D = new Context3D();

/**
 * @internal
 * Set the active WebGPU context. Called by Engine3D when activating an engine instance.
 */
