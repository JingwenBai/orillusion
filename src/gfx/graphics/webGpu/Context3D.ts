...
export class Context3D extends CEventDispatcher {
    ...
    // Shared GPU resources — initialized once, reused across all Engine3D instances
    ...
    /** Shared GPUAdapter across all Context3D instances */
    public get adapter(): GPUAdapter {
        return Context3D._sharedAdapter;
    }

    /** Shared GPUDevice across all Context3D instances */
    public get device(): GPUDevice {
        return Context3D._sharedDevice;
    }

    /** Shared presentation format across all Context3D instances */
     ...
     * On the first call the WebGPU adapter and device are created (shared across
     * all subsequent instances).  On every call a fresh GPUCanvasContext is
     * configured for the supplied (or auto-created) canvas element.
     */
    async init(canvasConfig?: CanvasConfig): Promise<boolean> {
        ...
        // Initialize shared GPU device only once across all instances
...
/**
 * @internal
 * The active WebGPU context for the currently rendering Engine3D instance.
 * Reassigned by Engine3D.activate() before each frame render.
 * All importing modules get the live value via ES-module named-export binding.
 */
export let webGPUContext: Context3D;

/**
 * @internal
 * Switch the active WebGPU context.  Called by Engine3D.activate().
 */
export function setActiveContext(ctx: Context3D) {
    webGPUContext = ctx;
}
