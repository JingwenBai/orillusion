...
export class Context3D extends CEventDispatcher {
    ...
    // Shared across all Engine3D instances (one device per page)
    ...
    async init(canvasConfig?: CanvasConfig): Promise<boolean> {
        ...
        if (Context3D._sharedDevice) {
            // Reuse the shared device so all Engine3D instances share one GPU device
 ...
 * Set by Engine3D before each render frame via setActiveContext().
 * All engine subsystems read from this live binding.
 * @internal
 */
