...
export class GPUContext {
    // Instance-level render state (per-engine)
    ...
    private static _active: GPUContext;

    public static setActive(instance: GPUContext) {
        this._active = instance;
    }

    /** Static accessor for lastRenderPassState — delegates to active instance */
    public static get lastRenderPassState(): RendererPassState {
        return this._active?.lastRenderPassState;
    }
    public static set lastRenderPassState(v: RendererPassState) {
        if (this._active) this._active.lastRenderPassState = v;
    }
    ...
    public static beginRenderPass(command: GPUCommandEncoder, renderPassState: RendererPassState): GPURenderPassEncoder {
        this.cleanCache();
        this._active.renderPassCount++;
        this._active.lastRenderPassState = renderPassState;
