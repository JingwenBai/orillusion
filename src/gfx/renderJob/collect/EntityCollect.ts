...
let _current: EntityCollect | null = null;

export function setCurrentEntityCollect(data: EntityCollect): void {
    _current = data;
}
...
export class EntityCollect {
    ...
    public engineSetting: import('../../../setting/EngineSetting').EngineSetting | null = null;
    ...
    public addRenderNode(root: Scene3D, renderNode: RenderNode) {
        ...
        if (renderNode.hasMask(RendererMask.Sky)) {
            ...
            if (this.engineSetting?.occlusionQuery?.octree) {
    ...
    private getOctree(root: Scene3D) {
        let octree: Octree;
        let setting = this.engineSetting?.occlusionQuery?.octree;
        if (setting) {
    ...
    public addLight(root: Scene3D, light: ILight) {
        if (!this._sceneLights.has(root)) {
            ...
            if (lights.length >= (this.engineSetting?.light?.maxLight ?? 4096)) {
                console.warn('Alreay meet maxmium light number:', (this.engineSetting?.light?.maxLight ?? 4096))
                return
            }
