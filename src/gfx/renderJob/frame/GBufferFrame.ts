...
export class GBufferFrame extends RTFrame {
    ...
    /**
     * @internal
     * Per-engine GBuffer registry. Engine3D activates its own map before each frame
     * via GBufferFrame.setActiveGBufferMap().  All existing call-sites continue to
     * use GBufferFrame.gBufferMap / getGBufferFrame() without modification.
     */
    private static _activeGBufferMap: Map<string, GBufferFrame>;

    /** @internal Backward-compat getter — returns the currently active map */
    public static get gBufferMap(): Map<string, GBufferFrame> {
        return this._activeGBufferMap;
    }

    /** @internal Called by Engine3D to swap in this engine's GBuffer registry */
    public static setActiveGBufferMap(map: Map<string, GBufferFrame>): void {
        this._activeGBufferMap = map;
    }
    ...
    public static getGBufferFrame(key: string, fixedWidth: number = 0, fixedHeight: number = 0, outColor: boolean = true, depthTexture?: RenderTexture): GBufferFrame {
        ...
        if (!GBufferFrame._activeGBufferMap.has(key)) {
            ...
            GBufferFrame._activeGBufferMap.set(key, gBuffer);
        } else {
            gBuffer = GBufferFrame._activeGBufferMap.get(key);
        }
        return gBuffer;
    }
