...
export class GBufferFrame extends RTFrame {
    ...
    // Per-engine G-buffer frame maps, keyed by engine ID.
    private static _engineMaps: Map<string, Map<string, GBufferFrame>> = new Map();
    private static _activeEngineId: string | null = null;

    /**
     * Switch the active engine. Called by Engine3D.activate() before every
     * render frame so that getGBufferFrame() resolves to the right engine's map.
     */
    public static setActiveEngine(id: string): void {
        if (!this._engineMaps.has(id)) {
            this._engineMaps.set(id, new Map<string, GBufferFrame>());
        }
        this._activeEngineId = id;
    }

    public static get gBufferMap(): Map<string, GBufferFrame> {
        if (!this._activeEngineId) {
            throw new Error('GBufferFrame: no active engine — call Engine3D.init() first.');
        }
