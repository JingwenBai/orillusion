...
export class GBufferFrame extends RTFrame {
    ...
    // GBuffer maps are keyed by CanvasContext so each Engine3D instance
    // maintains its own set of frame-buffers sized to its canvas.
    private static _gBufferMapsByCanvas: Map<CanvasContext, Map<string, GBufferFrame>> = new Map();

    /** @internal kept for backward compat – returns the map for the active canvas */
    public static get gBufferMap(): Map<string, GBufferFrame> {
        return GBufferFrame._getGBufferMap();
    }

    private static _getGBufferMap(): Map<string, GBufferFrame> {
        const ctx = webGPUContext.activeCanvas;
        let map = this._gBufferMapsByCanvas.get(ctx);
        if (!map) {
            map = new Map<string, GBufferFrame>();
            this._gBufferMapsByCanvas.set(ctx, map);
        }
        return map;
    }
    ...
    public static getGBufferFrame(key: string, fixedWidth: number = 0, fixedHeight: number = 0, outColor: boolean = true, depthTexture?: RenderTexture): GBufferFrame {
        const map = GBufferFrame._getGBufferMap();
