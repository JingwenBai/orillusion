...
export class GBufferFrame extends RTFrame {
    ...
    /**
     * Fallback map used before any engine is active (legacy / single-instance path).
     * @internal
     */
    private static _fallbackMap: Map<string, GBufferFrame> = new Map<string, GBufferFrame>();

    /**
     * Returns the GBuffer map for the currently active engine, or the fallback map.
     * @internal
     */
    private static _getMap(): Map<string, GBufferFrame> {
        const engine = getActiveEngine();
        return (engine?.gBufferMap as Map<string, GBufferFrame>) ?? GBufferFrame._fallbackMap;
    }
    ...
    /**
     * Get or create a GBuffer frame for the active engine.
     * Uses the active engine's per-instance gBufferMap to prevent
     * key collisions when multiple engines render to different canvases.
     * @internal
     */
