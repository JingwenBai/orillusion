...
export class GBufferFrame extends RTFrame {
    ...
    createGBuffer(key: string, rtWidth: number, rtHeight: number, autoResize: boolean = true, outColor: boolean = true, depthTexture?: RenderTexture, rtResourceMap?: RTResourceMap) {
        ...
        if (outColor) {
            ...
            this._colorBufferTex = rtResourceMap
                ? rtResourceMap.createRTTexture(key + RTResourceConfig.colorBufferTex_NAME, rtWidth, rtHeight, GPUTextureFormat.rgba16float, true)
                : new RenderTexture(rtWidth, rtHeight, GPUTextureFormat.rgba16float, true);
