...
export class GBufferFrame extends RTFrame {
    ...
    /**
     * The currently active GBuffer map for the rendering engine instance.
     * Set by Engine3D before each frame to enable per-instance isolation.
     */
    public static _activeMap: Map<string, GBufferFrame> | null = null;

    /** Default (first-instance) GBuffer map, retained for backward compat */
    private static _defaultMap: Map<string, GBufferFrame> = new Map<string, GBufferFrame>();

    /** @deprecated Access via Engine3D instance; kept for backward compat */
    public static get gBufferMap(): Map<string, GBufferFrame> {
        return GBufferFrame._activeMap ?? GBufferFrame._defaultMap;
    }
    ...
    public static getGBufferFrame(key: string, fixedWidth: number = 0, fixedHeight: number = 0, outColor: boolean = true, depthTexture?: RenderTexture): GBufferFrame {
        const map = GBufferFrame._activeMap ?? GBufferFrame._defaultMap;
