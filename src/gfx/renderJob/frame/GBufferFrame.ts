...
export class GBufferFrame extends RTFrame {
    ...
    /** @internal Active per-engine GBuffer registry */
    private static _activeGBufferMap: Map<string, GBufferFrame> = new Map<string, GBufferFrame>();

    /**
     * Per-engine GBufferFrame registry (backward-compat accessor).
     * For multi-instance use, call GBufferFrame.activate(map) before rendering.
     */
    public static get gBufferMap(): Map<string, GBufferFrame> {
        return GBufferFrame._activeGBufferMap;
    }

    public static activate(map: Map<string, GBufferFrame>): void {
        GBufferFrame._activeGBufferMap = map;
    }
