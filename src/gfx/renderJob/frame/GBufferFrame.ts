...
export class GBufferFrame extends RTFrame {
    ...
    /**
     * Returns the active engine's per-instance GBuffer map.
     * Replaces the former static gBufferMap property.
     * @internal
     */
    public static get gBufferMap(): Map<string, GBufferFrame> {
        return activeEngine?.gBufferFrames;
    }
    ...
    /**
     * Get or create a GBufferFrame for the active engine instance.
     * @internal
     */
    public static getGBufferFrame(key: string, fixedWidth: number = 0, fixedHeight: number = 0, outColor: boolean = true, depthTexture?: RenderTexture): GBufferFrame {
        const map = activeEngine?.gBufferFrames as Map<string, GBufferFrame>;
        if (!map) return null;
