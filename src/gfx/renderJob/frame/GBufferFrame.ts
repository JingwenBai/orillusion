...
export class GBufferFrame extends RTFrame {
    ...
    /** @internal active per-engine gBuffer map used by static API */
    public static _activeMap: Map<string, GBufferFrame> = new Map<string, GBufferFrame>();

    /** @deprecated access via Engine3D instance; static getter delegates to active map */
     ...
     * @internal
     */
    public static getGBufferFrame(key: string, fixedWidth: number = 0, fixedHeight: number = 0, outColor: boolean = true, depthTexture?: RenderTexture, map?: Map<string, GBufferFrame>): GBufferFrame {
        let activeMap = map ?? GBufferFrame._activeMap;
