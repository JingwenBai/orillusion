...
import { currentEnginePrefix } from "./EngineInstancePrefix";
...
export class GBufferFrame extends RTFrame {
    ...
    public static getGBufferFrame(key: string, fixedWidth: number = 0, fixedHeight: number = 0, outColor: boolean = true, depthTexture?: RenderTexture): GBufferFrame {
        const prefixedKey = currentEnginePrefix + key;
        let gBuffer: GBufferFrame;
        if (!GBufferFrame.gBufferMap.has(prefixedKey)) {
            ...
            GBufferFrame.gBufferMap.set(prefixedKey, gBuffer);
        } else {
            gBuffer = GBufferFrame.gBufferMap.get(prefixedKey);
        }
        return gBuffer;
    }
