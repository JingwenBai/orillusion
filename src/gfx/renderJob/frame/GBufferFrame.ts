...
export class GBufferFrame extends RTFrame {
    ...
    /**
     * The active per-engine GBuffer map. Switched by Engine3D before each render frame.
     * @internal
     */
    private static _activeGBufferMap: Map<string, GBufferFrame> = null;

    /**
     * Switch the active GBuffer map (called by Engine3D before each render frame).
     * @internal
     */
    public static setActive(gBufferMap: Map<string, GBufferFrame>) {
        GBufferFrame._activeGBufferMap = gBufferMap;
    }

    /**
     * Backward-compat accessor for the active engine's GBuffer map.
     */
    ...
    public static getGBufferFrame(key: string, fixedWidth: number = 0, fixedHeight: number = 0, outColor: boolean = true, depthTexture?: RenderTexture): GBufferFrame {
        const gBufferMap = GBufferFrame._activeGBufferMap;
        if (!gBufferMap) {
            throw new Error('No active GBufferFrame map. Make sure Engine3D.init() has been called.');
        }
