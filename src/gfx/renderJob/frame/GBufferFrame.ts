...
export class GBufferFrame extends RTFrame {
    ...
    createGBuffer(key: string, rtWidth: number, rtHeight: number, autoResize: boolean = true, outColor: boolean = true, depthTexture?: RenderTexture) {
        ...
        let compressGBufferRTDes = new RTDescriptor();
    ...
    /**
     * Returns (creating if needed) a GBufferFrame for the given key.
     * The lookup map is stored on the active Engine3D instance so each engine
     * keeps its own set of render textures sized to its own canvas.
     * @internal
     */
    ...
    /** Returns the per-engine GBufferFrame map from the active Engine3D instance. */
    private static _getMap(): Map<string, GBufferFrame> {
        return EngineContext.current.gBufferFrameMap as Map<string, GBufferFrame>;
    }
