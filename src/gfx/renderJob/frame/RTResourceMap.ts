 ...
 * Instantiated by Engine3D; static methods route to the current rendering instance.
 * @group Post
 */
export class RTResourceMap {
    ...
    public static _currentRenderingInstance: RTResourceMap | null = null;

    /** @internal Set by Engine3D on first init. */
    public static _defaultInstance: RTResourceMap | null = null;
    ...
    public static createRTTexture(name: string, rtWidth: number, rtHeight: number, format: GPUTextureFormat, useMipmap: boolean = false, sampleCount: number = 0): RenderTexture {
        return _get().createRTTexture(name, rtWidth, rtHeight, format, useMipmap, sampleCount);
    }
    ...
    public static createRTTextureArray(name: string, rtWidth: number, rtHeight: number, format: GPUTextureFormat, length: number = 1, useMipmap: boolean = false, sampleCount: number = 0): RenderTexture {
        return _get().createRTTextureArray(name, rtWidth, rtHeight, format, length, useMipmap, sampleCount);
    }
    ...
    public static createViewQuad(name: string, shaderVS: string, shaderFS: string, outRtTexture: RenderTexture, multisample: number = 0): ViewQuad {
        return _get().createViewQuad(name, shaderVS, shaderFS, outRtTexture, multisample);
    }

    public static getTexture(name: string): RenderTexture | undefined {
        return _get().getTexture(name);
    }

    public static CreateSplitTexture(id: string): RenderTexture {
        return _get().CreateSplitTexture(id);
    }

    public static WriteSplitColorTexture(id: string) {
        _get().WriteSplitColorTexture(id);
    }
}

function _get(): RTResourceMap {
    const inst = RTResourceMap._currentRenderingInstance ?? RTResourceMap._defaultInstance;
    if (!inst) throw new Error('RTResourceMap: no Engine3D instance initialised');
    return inst;
}
