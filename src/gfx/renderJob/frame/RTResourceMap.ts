...
export class RTResourceMap {
    ...
    public static init() {
        // No-op: maps are now per-engine, initialized lazily on Engine3D instances.
    }
    ...
    public static createRTTexture(name: string, rtWidth: number, rtHeight: number, format: GPUTextureFormat, useMipmap: boolean = false, sampleCount: number = 0) {
        const rtTextureMap: Map<string, RenderTexture> = getCurrentEngine()._rtTextureMap;
        let rt: RenderTexture = rtTextureMap.get(name);
        if (!rt) {
    ...
    public static createViewQuad(name: string, shaderVS: string, shaderFS: string, outRtTexture: RenderTexture, multisample: number = 0) {
        const rtViewQuad: Map<string, ViewQuad> = getCurrentEngine()._rtViewQuad;
        ...
        rtViewQuad.set(name, viewQuad);
        return viewQuad;
    }

    public static getTexture(name: string) {
        const rtTextureMap: Map<string, RenderTexture> = getCurrentEngine()._rtTextureMap;
        return rtTextureMap.get(name);
    }
