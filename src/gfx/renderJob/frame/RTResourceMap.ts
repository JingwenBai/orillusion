...
export class RTResourceMap {
    ...
    /**
     * Active instance used by static backward-compat methods.
     * Set by Engine3D before each render frame so that all existing
     * RTResourceMap.createRTTexture() calls route to the correct engine's pool.
     */
    public static active: RTResourceMap = (() => { const m = new RTResourceMap(); m.init(); return m; })();

    // Per-instance render texture pools
    ...
    // --- Static backward-compat methods (delegate to RTResourceMap.active) ---
    ...
    public static createRTTexture(name: string, rtWidth: number, rtHeight: number, format: GPUTextureFormat, useMipmap: boolean = false, sampleCount: number = 0): RenderTexture {
        return RTResourceMap.active.createRTTexture(name, rtWidth, rtHeight, format, useMipmap, sampleCount);
    }
    ...
    public static createRTTextureArray(name: string, rtWidth: number, rtHeight: number, format: GPUTextureFormat, length: number = 1, useMipmap: boolean = false, sampleCount: number = 0): RenderTexture {
        return RTResourceMap.active.createRTTextureArray(name, rtWidth, rtHeight, format, length, useMipmap, sampleCount);
    }
    ...
    public static createViewQuad(name: string, shaderVS: string, shaderFS: string, outRtTexture: RenderTexture, multisample: number = 0): ViewQuad {
        return RTResourceMap.active.createViewQuad(name, shaderVS, shaderFS, outRtTexture, multisample);
    }

    public static getTexture(name: string): RenderTexture {
        return RTResourceMap.active.rtTextureMap?.get(name);
    }

    public static CreateSplitTexture(id: string): RenderTexture {
        return RTResourceMap.active.CreateSplitTexture(id);
    }

    public static WriteSplitColorTexture(id: string): void {
        RTResourceMap.active.WriteSplitColorTexture(id);
    }

    /** @deprecated Use engine.rtResourceMap.init() instead */
    public static init() {
        RTResourceMap.active.init();
    }
}
