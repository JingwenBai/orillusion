...
export class RTResourceMap {
    ...
    private static get _instance(): RTResourceMap {
        return getCurrentEngine()?.rtResourceMap;
    }

    /** @internal */
    public static get rtTextureMap() { return this._instance?.rtTextureMap; }
    /** @internal */
    public static get rtViewQuad() { return this._instance?.rtViewQuad; }
    ...
    public static createRTTexture(name: string, rtWidth: number, rtHeight: number, format: GPUTextureFormat, useMipmap: boolean = false, sampleCount: number = 0) {
        return this._instance?.createRTTexture(name, rtWidth, rtHeight, format, useMipmap, sampleCount);
    }
    ...
    public static createRTTextureArray(name: string, rtWidth: number, rtHeight: number, format: GPUTextureFormat, length: number = 1, useMipmap: boolean = false, sampleCount: number = 0) {
        return this._instance?.createRTTextureArray(name, rtWidth, rtHeight, format, length, useMipmap, sampleCount);
    }

    /** @internal */
    public static createViewQuad(name: string, shaderVS: string, shaderFS: string, outRtTexture: RenderTexture, multisample: number = 0) {
        return this._instance?.createViewQuad(name, shaderVS, shaderFS, outRtTexture, multisample);
    }
    ...
    public static CreateSplitTexture(id: string) {
        return this._instance?.CreateSplitTexture(id);
    }

    /** @internal */
    public static WriteSplitColorTexture(id: string) {
        return this._instance?.WriteSplitColorTexture(id);
    }
}
