 ...
 * Static methods delegate to the currently active Engine3D instance's map
 * via the `activeRTResourceMap` live binding, so existing callers require
 * no changes.
 * @internal
 * @group Post
 */
export class RTResourceMap {
    ...
    /** @deprecated use instance constructor instead */
    public static init() {
        setActiveRTResourceMap(new RTResourceMap());
    }
    ...
    public static createRTTexture(name: string, rtWidth: number, rtHeight: number, format: GPUTextureFormat, useMipmap: boolean = false, sampleCount: number = 0): RenderTexture {
        return activeRTResourceMap._createRTTexture(name, rtWidth, rtHeight, format, useMipmap, sampleCount);
    }
    ...
    public static createRTTextureArray(name: string, rtWidth: number, rtHeight: number, format: GPUTextureFormat, length: number = 1, useMipmap: boolean = false, sampleCount: number = 0): RenderTexture {
        return activeRTResourceMap._createRTTextureArray(name, rtWidth, rtHeight, format, length, useMipmap, sampleCount);
    }
    ...
    public static createViewQuad(name: string, shaderVS: string, shaderFS: string, outRtTexture: RenderTexture, multisample: number = 0): ViewQuad {
        return activeRTResourceMap._createViewQuad(name, shaderVS, shaderFS, outRtTexture, multisample);
    }

    public static getTexture(name: string): RenderTexture {
        return activeRTResourceMap.rtTextureMap.get(name);
    }

    public static CreateSplitTexture(id: string): RenderTexture {
        return activeRTResourceMap._createSplitTexture(id);
    }

    public static WriteSplitColorTexture(id: string) {
        activeRTResourceMap._writeSplitColorTexture(id);
    }
...
/**
 * Live binding pointing to the active engine's RTResourceMap.
 * Reassigned by Engine3D.setActive() before each frame.
 * @internal
 */
export let activeRTResourceMap: RTResourceMap;

/** @internal */
export function setActiveRTResourceMap(map: RTResourceMap) {
    activeRTResourceMap = map;
}
