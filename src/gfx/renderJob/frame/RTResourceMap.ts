...
/**
 * @internal
 * Per-engine render-texture and ViewQuad registry.
 * Static methods are backward-compatible proxies that delegate to the
 * active Engine3D instance's rtResourceMap via Engine3D.current.
 * @group Post
 */
export class RTResourceMap {
    ...
    public static init() {
        Engine3D.current?.rtResourceMap?.init();
    }
    ...
    public static createRTTexture(name: string, rtWidth: number, rtHeight: number, format: GPUTextureFormat, useMipmap: boolean = false, sampleCount: number = 0): RenderTexture {
        return Engine3D.current?.rtResourceMap?.createRTTexture(name, rtWidth, rtHeight, format, useMipmap, sampleCount);
    }
    ...
    public static createRTTextureArray(name: string, rtWidth: number, rtHeight: number, format: GPUTextureFormat, length: number = 1, useMipmap: boolean = false, sampleCount: number = 0): RenderTexture {
        return Engine3D.current?.rtResourceMap?.createRTTextureArray(name, rtWidth, rtHeight, format, length, useMipmap, sampleCount);
    }
    ...
    public static createViewQuad(name: string, shaderVS: string, shaderFS: string, outRtTexture: RenderTexture, multisample: number = 0): ViewQuad {
        return Engine3D.current?.rtResourceMap?.createViewQuad(name, shaderVS, shaderFS, outRtTexture, multisample);
    }

    public static getTexture(name: string): RenderTexture {
        return Engine3D.current?.rtResourceMap?.getTexture(name);
    }

    public static WriteSplitColorTexture(id: string) {
        Engine3D.current?.rtResourceMap?.WriteSplitColorTexture(id);
    }

    public static CreateSplitTexture(id: string): RenderTexture {
        return Engine3D.current?.rtResourceMap?.CreateSplitTexture(id);
    }
