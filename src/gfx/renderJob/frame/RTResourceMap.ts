...
export class RTResourceMap {
    ...
    private static _resolve(): RTResourceMap {
        if (ActiveEngineContext.rtResourceMap instanceof RTResourceMap) return ActiveEngineContext.rtResourceMap;
        if (!RTResourceMap._default) RTResourceMap._default = new RTResourceMap();
        return RTResourceMap._default;
    }

    /** @internal */
    private static _default: RTResourceMap;
    ...
    public static createRTTexture(name: string, rtWidth: number, rtHeight: number, format: GPUTextureFormat, useMipmap: boolean = false, sampleCount: number = 0): RenderTexture {
        return this._resolve().createRTTexture(name, rtWidth, rtHeight, format, useMipmap, sampleCount);
    }
    ...
    public static createRTTextureArray(name: string, rtWidth: number, rtHeight: number, format: GPUTextureFormat, length: number = 1, useMipmap: boolean = false, sampleCount: number = 0): RenderTexture {
        return this._resolve().createRTTextureArray(name, rtWidth, rtHeight, format, length, useMipmap, sampleCount);
    }
    ...
    public static createViewQuad(name: string, shaderVS: string, shaderFS: string, outRtTexture: RenderTexture, multisample: number = 0): ViewQuad {
        return this._resolve().createViewQuad(name, shaderVS, shaderFS, outRtTexture, multisample);
    }

    public static getTexture(name: string): RenderTexture {
        return this._resolve().getTexture(name);
    }

    public static CreateSplitTexture(id: string): RenderTexture {
        return this._resolve().CreateSplitTexture(id);
    }

    public static WriteSplitColorTexture(id: string): void {
        this._resolve().WriteSplitColorTexture(id);
    }
}
