...
/** @internal */
export function setActiveRTResourceMap(map: RTResourceMap | null) {
    _activeInstance = map;
}
 ...
 * Each Engine3D instance holds its own RTResourceMap.
 * Static methods delegate to the active engine's instance so existing
 * render-pipeline code keeps working without modification.
 * @group Post
 */
export class RTResourceMap {
    ...
    public createRTTexture(
        name: string, rtWidth: number, rtHeight: number,
        format: GPUTextureFormat, useMipmap: boolean = false, sampleCount: number = 0
    ): RenderTexture {
    ...
    public createRTTextureArray(
        name: string, rtWidth: number, rtHeight: number,
        format: GPUTextureFormat, length: number = 1, useMipmap: boolean = false, sampleCount: number = 0
    ): RenderTexture {
        let rt: RenderTexture = this.rtTextureMap.get(name);
        if (!rt) {
    ...
    public createViewQuad(
        name: string, shaderVS: string, shaderFS: string,
        outRtTexture: RenderTexture, multisample: number = 0
    ): ViewQuad {
    ...
    public static get rtTextureMap(): Map<string, RenderTexture> {
        return _activeInstance?.rtTextureMap;
    }

    public static get rtViewQuad(): Map<string, ViewQuad> {
        return _activeInstance?.rtViewQuad;
    }
