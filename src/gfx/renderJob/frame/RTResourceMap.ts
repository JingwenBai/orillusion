...
let _current: RTResourceMap | null = null;

/**
 * @internal
 * Activate an RTResourceMap instance as the current one. Called by Engine3D.
 */
export function _setCurrentRTResourceMap(m: RTResourceMap | null): void {
    _current = m;
}
...
export class RTResourceMap {
    ...
    // ── Static accessors (delegate to current engine's RTResourceMap) ─────────
    ...
    public static createRTTexture(name: string, rtWidth: number, rtHeight: number, format: GPUTextureFormat, useMipmap: boolean = false, sampleCount: number = 0) {
        let rt: RenderTexture = _current!.rtTextureMap.get(name);
        if (!rt) {
            ...
            rt.name = name;
            _current!.rtTextureMap.set(name, rt);
        }
        return rt;
    }
    ...
    public static createViewQuad(name: string, shaderVS: string, shaderFS: string, outRtTexture: RenderTexture, multisample: number = 0) {
        ...
        _current!.rtViewQuad.set(name, viewQuad);
        return viewQuad;
    }

    public static getTexture(name: string) {
        return _current!.rtTextureMap.get(name);
    }
