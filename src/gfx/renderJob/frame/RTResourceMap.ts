...
export class RTResourceMap {
    ...
    // Per-engine maps stored here; keyed by engine instance id
    private static _engineMaps: Map<number, { rtTextureMap: Map<string, RenderTexture>; rtViewQuad: Map<string, ViewQuad> }> = new Map();
    // Current active engine id (set by Engine3D before each frame / during init)
    public static _currentEngineId: number = 0;

    private static get _maps() {
        let m = RTResourceMap._engineMaps.get(RTResourceMap._currentEngineId);
        if (!m) {
            m = { rtTextureMap: new Map<string, RenderTexture>(), rtViewQuad: new Map<string, ViewQuad>() };
            RTResourceMap._engineMaps.set(RTResourceMap._currentEngineId, m);
        }
        return m;
    }

    public static get rtTextureMap(): Map<string, RenderTexture> {
        return RTResourceMap._maps.rtTextureMap;
    }

    public static get rtViewQuad(): Map<string, ViewQuad> {
        return RTResourceMap._maps.rtViewQuad;
    }

    public static init() {
        // Initialize maps for the current engine (creates fresh maps for each engine instance)
        RTResourceMap._engineMaps.set(RTResourceMap._currentEngineId, {
    ...
    public static createRTTexture(name: string, rtWidth: number, rtHeight: number, format: GPUTextureFormat, useMipmap: boolean = false, sampleCount: number = 0) {
        const maps = RTResourceMap._maps;
    ...
    public static createViewQuad(name: string, shaderVS: string, shaderFS: string, outRtTexture: RenderTexture, multisample: number = 0) {
        ...
        maps.rtViewQuad.set(name, viewQuad);
        return viewQuad;
    }

    public static getTexture(name: string) {
        return RTResourceMap._maps.rtTextureMap.get(name);
    }
