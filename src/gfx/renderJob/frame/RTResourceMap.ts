...
import { getActiveEngineRenderMaps } from '../ActiveEngineContext';

/**
 * @internal
 * @group Post
 *
 * All state is now stored per-Engine3D instance inside EngineRenderMaps.
 * The static methods here delegate to whichever engine is currently active
 * (set via setActiveEngineRenderMaps before each frame / during engine init).
 */
export class RTResourceMap {

    public static get rtTextureMap(): Map<string, RenderTexture> {
        return getActiveEngineRenderMaps().rtTextureMap;
    }

    public static get rtViewQuad(): Map<string, ViewQuad> {
        return getActiveEngineRenderMaps().rtViewQuad;
    }

    /** No-op – maps are now owned by Engine3D instances. */
    public static init() {}
    ...
    public static createRTTexture(name: string, rtWidth: number, rtHeight: number, format: GPUTextureFormat, useMipmap: boolean = false, sampleCount: number = 0) {
        let map = RTResourceMap.rtTextureMap;
