 ...
 * Each Engine3D instance holds its own RTResourceMap to avoid cross-engine name conflicts.
 * @internal
 * @group Post
 */
export class RTResourceMap {

    // ── Static backward-compat singleton ─────────────────────────────────────
    private static _instance: RTResourceMap;
    /** Set by Engine3D.init() to point to the current engine's resource map. */
    public static get instance(): RTResourceMap {
        if (!this._instance) this._instance = new RTResourceMap();
        return this._instance;
    }
    public static set instance(value: RTResourceMap) {
    ...
    public static createRTTexture(name: string, rtWidth: number, rtHeight: number, format: GPUTextureFormat, useMipmap: boolean = false, sampleCount: number = 0): RenderTexture {
        return RTResourceMap.instance.createRTTexture(name, rtWidth, rtHeight, format, useMipmap, sampleCount);
    }
    public static createRTTextureArray(name: string, rtWidth: number, rtHeight: number, format: GPUTextureFormat, length: number = 1, useMipmap: boolean = false, sampleCount: number = 0): RenderTexture {
        return RTResourceMap.instance.createRTTextureArray(name, rtWidth, rtHeight, format, length, useMipmap, sampleCount);
    }
    public static createViewQuad(name: string, shaderVS: string, shaderFS: string, outRtTexture: RenderTexture, multisample: number = 0): ViewQuad {
        return RTResourceMap.instance.createViewQuad(name, shaderVS, shaderFS, outRtTexture, multisample);
    }
    public static getTexture(name: string): RenderTexture {
        return RTResourceMap.instance.getTexture(name);
    }
    public static CreateSplitTexture(id: string): RenderTexture {
        return RTResourceMap.instance.CreateSplitTexture(id);
    }
    public static WriteSplitColorTexture(id: string): void {
        RTResourceMap.instance.WriteSplitColorTexture(id);
    }
    // ─────────────────────────────────────────────────────────────────────────
    ...
    /** Reset all cached resources (called on engine re-init). */
    public init() {
