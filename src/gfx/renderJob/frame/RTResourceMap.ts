...
export class RTResourceMap {
    ...
    // -------------------------------------------------------------------------
    // Static facade — routes calls to the active engine's RTResourceMap.
    // All existing code calling RTResourceMap.createRTTexture() is unchanged.
    // -------------------------------------------------------------------------

    private static _fallback: RTResourceMap = new RTResourceMap();

    private static _getInstance(): RTResourceMap {
        const engine = getActiveEngine();
        return (engine?.rtResourceMap as RTResourceMap) ?? RTResourceMap._fallback;
    }

    public static init() {
        RTResourceMap._getInstance().init();
    }
    ...
    public static createRTTexture(name: string, rtWidth: number, rtHeight: number, format: GPUTextureFormat, useMipmap: boolean = false, sampleCount: number = 0): RenderTexture {
        return RTResourceMap._getInstance().createRTTexture(name, rtWidth, rtHeight, format, useMipmap, sampleCount);
    }
    ...
    public static createRTTextureArray(name: string, rtWidth: number, rtHeight: number, format: GPUTextureFormat, length: number = 1, useMipmap: boolean = false, sampleCount: number = 0): RenderTexture {
        return RTResourceMap._getInstance().createRTTextureArray(name, rtWidth, rtHeight, format, length, useMipmap, sampleCount);
    }
    ...
    public static createViewQuad(name: string, shaderVS: string, shaderFS: string, outRtTexture: RenderTexture, multisample: number = 0): ViewQuad {
        return RTResourceMap._getInstance().createViewQuad(name, shaderVS, shaderFS, outRtTexture, multisample);
    }

    public static getTexture(name: string): RenderTexture {
        return RTResourceMap._getInstance().getTexture(name);
    }

    public static CreateSplitTexture(id: string): RenderTexture {
        return RTResourceMap._getInstance().CreateSplitTexture(id);
    }

    public static WriteSplitColorTexture(id: string) {
        RTResourceMap._getInstance().WriteSplitColorTexture(id);
    }
}
