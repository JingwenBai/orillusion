...
export class RTResourceMap {
    ...
    /**
     * Currently active RTResourceMap instance (set by Engine3D before rendering)
     * @internal
     */
    public static _active: RTResourceMap;

    /**
     * Set the active RTResourceMap instance for the current Engine3D context.
     * @internal
     */
    ...
    // --------------- Static backward-compat API (delegates to active instance) ---------------
    ...
    public static createRTTexture(name: string, rtWidth: number, rtHeight: number, format: GPUTextureFormat, useMipmap: boolean = false, sampleCount: number = 0) {
        let map = RTResourceMap._active.rtTextureMap;
