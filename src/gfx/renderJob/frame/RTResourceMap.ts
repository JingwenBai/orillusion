...
import { getCurrentEngine } from '../../../core/EngineRegistry';
...
export class RTResourceMap {
    ...
    // ---- Static compat API (delegates to the current engine's RTResourceMap) ----

    private static _getActive(): RTResourceMap {
        const engine = getCurrentEngine();
        if (!engine || !engine.rtResourceMap) throw new Error('RTResourceMap: no active Engine3D instance');
        return engine.rtResourceMap;
    }

    /** @internal No-op — instance is created in Engine3D.init() */
    public static init(): void { /* no-op */ }
    ...
    public static createRTTextureArray(name: string, rtWidth: number, rtHeight: number, format: GPUTextureFormat, length: number = 1, useMipmap: boolean = false, sampleCount: number = 0): RenderTexture {
        return RTResourceMap._getActive().createRTTextureArray(name, rtWidth, rtHeight, format, length, useMipmap, sampleCount);
    }
