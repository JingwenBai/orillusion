...
export class SSRPost extends PostBase {
    ...
    private createRayTraceShader() {
        ...
        const _sky = EntityCollect.instance.getSky(this.view?.scene ?? this.scene);
        if (_sky instanceof SkyRenderer)
            this.SSR_RayTraceCompute.setSamplerTexture(`prefilterMap`, _sky.map);
