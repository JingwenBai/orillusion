...
export class DDGIProbeRenderer extends RendererBase {
    ...
    public updateProbe(view: View3D, probe: Probe, encoder: GPURenderPassEncoder,) {
        let lights = view.engine?.entityCollect?.getLights(view.scene) ?? [];
    ...
    private renderSceneOnce(view: View3D, probeCamera: Camera3D, encoder: GPURenderPassEncoder, lights: ILight[]) {
        ...
        let collectInfo = view.engine?.entityCollect?.getRenderNodes(view.scene, probeCamera);
    ...
    public render(view: View3D, occlusionSystem: OcclusionSystem) {
        ...
        if (view.engine?.entityCollect?.state?.giLightingChange || probeBeRendered || Engine3D.setting.gi.realTimeGI) {
            if (view.engine?.entityCollect) view.engine.entityCollect.state.giLightingChange = false;
    ...
    private rendProbe(view: View3D): void {
        ...
        if (execRender) {
            let probeList = view.engine?.entityCollect?.getProbes(view.scene) ?? [];
