...
// Called once per Engine3D instance so each instance gets independent defaults.
 ...
 *
 * Can be used either as a classic static singleton (backwards compatible):
 * ```ts
 * await Engine3D.init({ canvasConfig });
 * Engine3D.startRenderView(view);
 * ```
 *
 * Or instantiated directly for multi-instance setups:
 * ```ts
 * const engine1 = new Engine3D();
 * const engine2 = new Engine3D();
 * await engine1.init({ canvasConfig: config1 });
 * await engine2.init({ canvasConfig: config2 });
...
export class Engine3D {
    ...
    /** engine settings (each instance has independent settings) */
    ...
    /** @internal per-engine render-target texture map */
    public rtResourceMap: RTResourceMap;

    /** @internal per-engine GBuffer map (avoids string-key collisions across instances) */
    ...
    // ── One-time global guards ───────────────────────────────────────────────
    private static _wasmInitialized: boolean = false;
    private static _gpuContextInitialized: boolean = false;
    private static _shaderInitialized: boolean = false;

    // ── Default (backwards-compat) instance ─────────────────────────────────
    // Created eagerly so Engine3D.setting.xxx = ... works before init().
    private static _default: Engine3D = new Engine3D();

    // ── Helper: active engine for static facade ──────────────────────────────
    private static get _active(): Engine3D {
        return (EngineContext.current as Engine3D) ?? Engine3D._default;
    }

    // ── Static getters/setters (backwards-compatible facade) ─────────────────

    /** resource manager in engine3d */
    public static get res(): Res { return Engine3D._active?.res; }
    public static set res(v: Res) { const e = Engine3D._active; if (e) e.res = v; }

    /** input system in engine3d */
    public static get inputSystem(): InputSystem { return Engine3D._active?.inputSystem; }
    public static set inputSystem(v: InputSystem) { const e = Engine3D._active; if (e) e.inputSystem = v; }

    /** more view in engine3d */
    public static get views(): View3D[] { return Engine3D._active?.views; }
    public static set views(v: View3D[]) { const e = Engine3D._active; if (e) e.views = v; }

    /**
     * engine setting – in a multi-instance setup this returns the setting of the
     * currently active engine (set by EngineContext), falling back to the default instance.
     */
    public static get setting(): EngineSetting { return Engine3D._active?.setting; }
    public static set setting(v: EngineSetting) { const e = Engine3D._active; if (e) e.setting = v; }

    /** @internal */
    public static get renderJobs(): Map<View3D, RendererJob> { return Engine3D._active?.renderJobs; }
    public static set renderJobs(v: Map<View3D, RendererJob>) { const e = Engine3D._active; if (e) e.renderJobs = v; }
    ...
    // ── Static init (backwards-compatible) ──────────────────────────────────

    /**
     * Create and initialise the default engine instance.
     * Equivalent to `new Engine3D().init(descriptor)` for single-engine apps.
     */
    public static async init(descriptor: { canvasConfig?: CanvasConfig; beforeRender?: Function; renderLoop?: Function; lateRender?: Function, engineSetting?: EngineSetting } = {}) {
        ...
        // Reuse the eagerly-created default instance
     ...
     * Calling this on a second Engine3D shares the GPU device and shader library
     * but gives the instance its own resource manager, entity collect, and render targets.
     */
    public async init(descriptor: { canvasConfig?: CanvasConfig; beforeRender?: Function; renderLoop?: Function; lateRender?: Function, engineSetting?: EngineSetting } = {}) {
        // Mark this engine as the currently active one
        EngineContext.setCurrent(this);

        this.setting = { ...createDefaultSetting(), ...descriptor.engineSetting };

        // WASM matrix library is global – only initialise once
        if (!Engine3D._wasmInitialized) {
            await WasmMatrix.init(Matrix4.allocCount, this.setting.doublePrecision);
            Engine3D._wasmInitialized = true;
        }

        // GPU context is global – only initialise once
        if (!Engine3D._gpuContextInitialized) {
            await webGPUContext.init(descriptor.canvasConfig);
            Engine3D._gpuContextInitialized = true;
        }
        ...
        // Create per-engine systems before anything that may call them
        this.entityCollect = new EntityCollect();
        this.rtResourceMap = new RTResourceMap();
        // gBufferMap is already initialised as an empty Map in the field initialiser
        ...
        // Shader libraries are global – only initialise once
        ...
        // GlobalBindGroup and ShadowLightsCollect guard themselves
    ...
    // ── Instance render-view API ─────────────────────────────────────────────

    /**
     * Set render view and start renderer (instance version)
     */
    public startRenderView(view: View3D): RendererJob {
        // Bind engine to view (and to the scene if already assigned)
        view.engine = this;
    ...
    public startRenderViews(views: View3D[]): void {
        ...
        for (let i = 0; i < views.length; i++) {
            const v = views[i];
            v.engine = this;
            if (v.scene) v.scene.engine = this;
            this._startRenderJob(v);
        }
        this.resume();
    }

    /**
     * Get view render job instance (instance version)
     */
    ...
    // ── Internal render loop (instance) ─────────────────────────────────────

    private async _render(time: number): Promise<void> {
        // Establish this engine as the active context for all static accessors
        EngineContext.setCurrent(this);
