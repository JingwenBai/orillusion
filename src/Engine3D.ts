...
import { newEngineId, setCurrentEngineId } from './core/EngineContext';
...
export class Engine3D {
    ...
    private static _preInitSetting: EngineSetting = createDefaultSetting();
    ...
    public static get setting(): EngineSetting {
        return Engine3D.current?.setting ?? Engine3D._defaultInstance?.setting ?? Engine3D._preInitSetting;
    }

    public static set setting(value: EngineSetting) {
        if (Engine3D._defaultInstance) {
            Engine3D._defaultInstance.setting = value;
        } else {
            Engine3D._preInitSetting = value;
        }
    }
    ...
    public static get size(): number[] {
        return (Engine3D.current?.webGPUContext ?? webGPUContext).presentationSize;
    }
    ...
    public static get aspect(): number {
        return (Engine3D.current?.webGPUContext ?? webGPUContext).aspect;
    }
    ...
    public static get width(): number {
        return (Engine3D.current?.webGPUContext ?? webGPUContext).windowWidth;
    }
    ...
    public static get height(): number {
        return (Engine3D.current?.webGPUContext ?? webGPUContext).windowHeight;
    }
    ...
    public static async init(descriptor: { canvasConfig?: CanvasConfig; beforeRender?: Function; renderLoop?: Function; lateRender?: Function, engineSetting?: EngineSetting } = {}) {
        if (!Engine3D._defaultInstance) {
            ...
            Engine3D._defaultInstance.setting = { ...Engine3D._preInitSetting, ...descriptor.engineSetting };
        }
    ...
    public static startRenderViews(views: View3D[]) {
        return Engine3D._defaultInstance?.startRenderViews(views);
    }
    ...
    // ---- Instance methods (primary multi-instance API) ----
    ...
    /**
     * create webgpu 3d engine (instance method)
     * @param descriptor  {@link CanvasConfig}
     * @returns
     */
    ...
    /**
     * Pause the engine render (instance method)
     */
    public pause() {
    ...
    /**
     * Resume the engine render (instance method)
     */
    public resume() {
        if (this._requestAnimationFrameID === 0)
