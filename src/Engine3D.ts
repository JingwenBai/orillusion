...
import { setActiveEngineContext } from './EngineRegistry';
 ...
 *
 * Supports multiple independent instances — each instance maintains its own
 * WebGPU context, component lists, render jobs and all other per-engine state.
 *
 * Instance usage (multi-instance):
 *   const engine = new Engine3D();
 *   await engine.init({ canvasConfig: { canvas } });
 *   engine.startRenderView(view);
 *
 * Static usage (backward-compatible, single default instance):
 *   await Engine3D.init();
...
export class Engine3D {
     ...
     * resource manager
     */
    public get res(): Res { return this.context.res; }

    /**
     * input system
     */
    public get inputSystem(): InputSystem { return this.context.inputSystem; }

    /**
     * active views
     */
    public get views(): View3D[] { return this.context.views; }
     ...
     * @internal
     */
    public get renderJobs(): Map<View3D, RendererJob> { return this.context.renderJobs; }
    ...
    public get size(): number[] { return this.context.webGPUContext.presentationSize; }
    public get aspect(): number { return this.context.webGPUContext.aspect; }
    public get width(): number { return this.context.webGPUContext.windowWidth; }
    public get height(): number { return this.context.webGPUContext.windowHeight; }

    /**
     * Initialize this engine instance.
     * Sets this instance as the active engine context for all internal static utilities.
     */
    public async init(descriptor: {
        ...
        // Activate this engine's context so all static utilities resolve to it
        setActiveEngineContext(this.context);
        ...
        this.context.res = new Res();
        this.context.res.initDefault();

        this._beforeRender = descriptor.beforeRender;
        this._renderLoop = descriptor.renderLoop;
        this._lateRender = descriptor.lateRender;
        this.context.inputSystem = new InputSystem();
        this.context.inputSystem.initCanvas(this.context.webGPUContext.canvas);
    }

    private _startRenderJob(view: View3D): RendererJob {
        // Ensure this engine is active before touching any static subsystems
        setActiveEngineContext(this.context);
        let renderJob = new ForwardRenderJob(view);
        this.context.renderJobs.set(view, renderJob);
    ...
    public getRenderJob(view: View3D): RendererJob {
        return this.context.renderJobs.get(view);
    }
    ...
    private async _render(time: number): Promise<void> {
        // Make this engine the active context for the entire frame
        setActiveEngineContext(this.context);
    ...
    private async _updateFrame(time: number): Promise<void> {
        ...
        ctx.time = time;
        ctx.frame += 1;
        Interpolator.tick(ctx.delta);

        const views = ctx.views;
        for (let i = 0; i < views.length; i++) {
            const view = views[i];
            view.scene.waitUpdate();
            let [w, h] = ctx.webGPUContext.presentationSize;
        ...
        let command = ctx.webGPUContext.device.createCommandEncoder();
        ...
        ctx.webGPUContext.device.queue.submit([command.finish()]);
    ...
    // ─── Static backward-compatible API (delegates to default instance) ────────
    ...
    /** @deprecated Use instance API: `new Engine3D()` */
    ...
    public static set frameRate(value: number) { Engine3D._getDefault().frameRate = value; }
