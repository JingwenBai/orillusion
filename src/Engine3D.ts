...
export class Engine3D {
    ...
    /**
     * Engine-wide settings shared by all instances.
     * Override per-instance by assigning `engine.setting` after construction.
     */
    ...
    // ─── Static backward-compat registry ────────────────────────────────────

    /** The first Engine3D instance created via the static `init()` API. */
    private static _defaultInstance: Engine3D | null = null;

    // Static backward-compat property accessors — delegate to the default instance.

    public static get res(): Res {
        return Engine3D._defaultInstance?.res;
    }
    public static set res(v: Res) {
        if (Engine3D._defaultInstance) Engine3D._defaultInstance.res = v;
    }
    ...
    public static set inputSystem(v: InputSystem) {
        if (Engine3D._defaultInstance) Engine3D._defaultInstance.inputSystem = v;
    }
    ...
    /** Input system for this engine instance (bound to its canvas). */
    public inputSystem: InputSystem;

    /** Active views being rendered by this instance. */
    public views: View3D[];

    /** The canvas context owned by this engine instance. */
    public canvasContext: CanvasContext;

    /** @internal */
    public renderJobs: Map<View3D, RendererJob>;

    /** Per-instance setting override. Defaults to the shared static `Engine3D.setting`. */
    ...
    // ─── Instance canvas-size helpers ────────────────────────────────────────

    get size(): number[] {
        return this.canvasContext?.presentationSize ?? [0, 0];
    }
    get aspect(): number {
        return this.canvasContext?.aspect ?? 1;
    }
    get width(): number {
        return this.canvasContext?.windowWidth ?? 0;
    }
    get height(): number {
        return this.canvasContext?.windowHeight ?? 0;
    }
     ...
     *
     * The first call to `init()` (on any instance) triggers shared one-time
     * initialisation: WebGPU device creation, WASM matrix library, shader
     * registry, global bind groups, and render-target resource map.
     *
     * Subsequent calls only set up the per-instance canvas and resources.
     */
    async init(descriptor: {
        ...
        // ── One-time shared initialisation ──────────────────────────────────
        if (!webGPUContext.device) {
            await WasmMatrix.init(Matrix4.allocCount, this.setting.doublePrecision);
            await webGPUContext.initGPU();

            // Pre-compute reflection settings once
        ...
        // ── Per-instance canvas ──────────────────────────────────────────────
        this.canvasContext = await webGPUContext.createCanvasContext(descriptor.canvasConfig);
        webGPUContext.setActiveCanvas(this.canvasContext);
    ...
    private async _render(time: number): Promise<void> {
        // Activate this engine's canvas so all canvas-specific getters on
        // webGPUContext return values for the correct canvas.
        webGPUContext.setActiveCanvas(this.canvasContext);
    ...
    //
    // All static methods create/use a single "default" instance so that
    // existing single-instance code continues to work without modification.

    /**
     * Create and initialise the default engine instance (legacy API).
     * Returns the Engine3D instance for optional use in multi-instance code.
     */
    ...
    /** Set a single render view and start the default engine's render loop. */
    public static startRenderView(view: View3D): RendererJob {
        return Engine3D._defaultInstance?.startRenderView(view);
    }

    /** Set multiple render views and start the default engine's render loop. */
    public static startRenderViews(views: View3D[]): void {
        Engine3D._defaultInstance?.startRenderViews(views);
    }

    /** Get the render job for a view from the default engine. */
