...
/**
 * Default engine settings shared by all Engine3D instances unless overridden
 * on a specific instance via `engineInstance.setting`.
 * @internal
 */
 ...
 *
 * Supports **multiple simultaneous instances**, each with its own canvas and scene.
 *
...
export class Engine3D {
    ...
    // =========================================================
    //  STATIC: shared configuration & backward-compat proxies
    // =========================================================

    /**
     * Engine settings. In single-instance projects this is the primary config
     * object. In multi-instance projects each engine exposes its own `setting`
     * property; this static field mirrors the first engine's setting for code
     * that accesses `Engine3D.setting` directly.
     */
    public static setting: EngineSetting = { ..._defaultSetting } as EngineSetting;

    /** All Engine3D instances created in the current session. */
    private static _instances: Engine3D[] = [];

    // ---- static backward-compat property proxies (delegate to first instance) ----

    /**
     * resource manager in engine3d
     */
    public static get res(): Res { return Engine3D._instances[0]?.res; }
    public static set res(v: Res) { if (Engine3D._instances[0]) Engine3D._instances[0].res = v; }

    /**
     * input system in engine3d
     */
    public static get inputSystem(): InputSystem { return Engine3D._instances[0]?.inputSystem; }
    public static set inputSystem(v: InputSystem) { if (Engine3D._instances[0]) Engine3D._instances[0].inputSystem = v; }

    /**
     * active views in engine3d
     */
    public static get views(): View3D[] { return Engine3D._instances[0]?.views; }
    public static set views(v: View3D[]) { if (Engine3D._instances[0]) Engine3D._instances[0].views = v; }

    /** @internal */
    public static get renderJobs(): Map<View3D, RendererJob> { return Engine3D._instances[0]?.renderJobs; }
    public static set renderJobs(v: Map<View3D, RendererJob>) { if (Engine3D._instances[0]) Engine3D._instances[0].renderJobs = v; }

    /**
     * set engine render frameRate 24/30/60/114/120/144/240/360 fps or other
     */
    public static get frameRate(): number {
        return Engine3D._instances[0]?._frameRate ?? 360;
    }
    public static set frameRate(value: number) {
        if (Engine3D._instances[0]) Engine3D._instances[0].frameRate = value;
    }
    ...
    // ---- static methods (backward-compat: delegate to first instance) ----

    /**
     * Create a default Engine3D instance and initialise it.
     * Equivalent to `new Engine3D()` followed by `instance.init(descriptor)`.
     */
    ...
    public static startRenderView(view: View3D) {
        return Engine3D._instances[0]?.startRenderView(view);
    }
    ...
    public static startRenderViews(views: View3D[]) {
        return Engine3D._instances[0]?.startRenderViews(views);
    }
    ...
    public static getRenderJob(view: View3D): RendererJob {
        return Engine3D._instances[0]?.getRenderJob(view);
    }

    /** Pause the default engine render */
    public static pause() { Engine3D._instances[0]?.pause(); }

    /** Resume the default engine render */
    public static resume() { Engine3D._instances[0]?.resume(); }
    ...
    /**
     * Per-instance engine settings. Defaults to a copy of Engine3D.setting.
     * Override before calling init() to customise this engine's behaviour.
     */
    ...
    // Per-engine render-resource registries
    private _rtResourceMap: RTResourceMap;
    private _gBufferMap: Map<string, GBufferFrame>;

    constructor() {
        this.setting = { ...Engine3D.setting } as EngineSetting;
    ...
    public get size(): number[] { return this.context3D?.presentationSize ?? [0, 0]; }
    public get aspect(): number { return this.context3D?.aspect ?? 1; }
    public get width(): number { return this.context3D?.windowWidth ?? 0; }
    public get height(): number { return this.context3D?.windowHeight ?? 0; }

    // ---- instance methods ----

    /**
     * Activate this engine as the current rendering context.
     * Updates all shared/static subsystems (webGPUContext, RTResourceMap,
     * GBufferFrame) to point at this engine's resources. Must be called before
     * any rendering or resource-creation code runs on behalf of this engine.
     * @internal
     */
     ...
     * @param descriptor  {@link CanvasConfig} and optional callbacks / settings.
     */
    public async init(descriptor: { canvasConfig?: CanvasConfig; beforeRender?: Function; renderLoop?: Function; lateRender?: Function, engineSetting?: EngineSetting } = {}) {
        ...
        // Merge supplied settings into this instance's copy.
        // Also update the shared static setting so Engine3D.setting continues to
        // work in code that doesn't have a reference to the engine instance.
        if (descriptor.engineSetting) {
            this.setting = { ...this.setting, ...descriptor.engineSetting };
            if (Engine3D._instances[0] === this) {
                Engine3D.setting = this.setting;
            }
        }

        // WasmMatrix is global (one WASM module, one shared transform buffer).
        // Initialise only once; subsequent engines reuse the same buffer.
        if (!WasmMatrix.wasm) {
            await WasmMatrix.init(Matrix4.allocCount, this.setting.doublePrecision);
        }

        // Create and activate this engine's WebGPU context.
        // Context3D will reuse the shared GPUAdapter/GPUDevice that was created
        // by the first engine, giving every engine the same device so that
        // shader modules and pipeline caches remain valid for all of them.
        ...
        // ShaderUtil, GlobalBindGroup, and ShadowLightsCollect guard against
        // double-initialisation internally so it is safe to call them here for
    ...
    private async _updateFrame(time: number) {
        // Activate this engine's resources before any subsystem reads them.
        this.setActive();
