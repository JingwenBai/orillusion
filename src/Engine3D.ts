...
import { setCurrentEngine, getCurrentEngine } from './EngineContext';

/**
 * Default engine settings template — shared as initial values, copied per instance.
 * @internal
 */
 ...
 *
 * Create one instance per canvas:
 * ```ts
 ...
 * ```
 *
 * For single-instance code the original static API still works via static
 * accessors that delegate to `Engine3D.current` (the most recently activated
 * engine instance).
 *
 * @group engine3D
 */
export class Engine3D {
    ...
    /**
     * The most recently activated engine instance.  All static accessor
     * properties delegate here, which keeps existing single-instance code
     * working unchanged while multiple engines can coexist.
     */
    public static get current(): Engine3D {
        return getCurrentEngine();
    }

    // ─── Backward-compat static accessors ──────────────────────────────────────

    /** @deprecated Use instance.res — static accessor delegates to Engine3D.current */
    ...
    public static set setting(value: EngineSetting) {
        if (Engine3D.current) Engine3D.current.setting = value;
    }
    ...
    // ─── Static method shims (single-instance backward compat) ─────────────────
    ...
    public static async init(descriptor: {
        ...
        // When called statically, create a default engine instance
        const engine = new Engine3D();
     ...
     * Multiple instances share the same GPUDevice but have separate canvas contexts.
     */
    ...
    /** @internal */ public entityCollect: EntityCollect;
    ...
    /**
     * Activate this engine — sets it as the current engine and switches
     * the active WebGPU context. Called automatically before each render pass.
     */
    ...
    public async init(descriptor: {
        ...
        // Activate before init so subsystem factories (GlobalBindGroup etc.) resolve correctly
        this.activate();

        // WasmMatrix is a shared WASM singleton; init once for the whole application
        if (!WasmMatrix.matrixStateBuffer) {
            await WasmMatrix.init(Matrix4.allocCount, this.setting.doublePrecision);
        }

        this.context = new Context3D();
        await this.context.init(descriptor.canvasConfig);
        // Keep the module-level reference current
        setActiveWebGPUContext(this.context);
        ...
        // ShaderLib / ShaderUtil are device-level singletons; initialise only once
        ShaderLib.init(); // idempotent — register() checks before overwriting
        if (!ShaderUtil.renderShaderModulePool) {
            ShaderUtil.init();
        }
    ...
    private async _updateFrame(time: number) {
        // Activate this engine so all static subsystem accessors resolve to us
        this.activate();
