 ...
 *
 * Supports multiple independent instances. Create an instance and call init():
 *
 *   const engine = new Engine3D();
 *   await engine.init({ canvasConfig: { canvas } });
 *   engine.startRenderView(view);
 *
 * The legacy all-static API is preserved for backward compatibility via
 * delegation to a shared default instance.
 *
 * @group engine3D
 */
export class Engine3D {
    ...
    public get size(): number[] {
        return this.context.webGPUContext.presentationSize;
    }

    public get aspect(): number {
        return this.context.webGPUContext.aspect;
    }

    public get width(): number {
        return this.context.webGPUContext.windowWidth;
    }

    public get height(): number {
        return this.context.webGPUContext.windowHeight;
    }

    /**
     * Initialize the engine with a canvas and settings.
     */
    public async init(descriptor: { canvasConfig?: CanvasConfig; beforeRender?: Function; renderLoop?: Function; lateRender?: Function, engineSetting?: EngineSetting } = {}): Promise<void> {
        ...
        // Phase 1: create non-GPU context and activate it
        ...
        // Phase 2: initialize GPU-dependent resources now that the device is ready
        this.context.initGPU();
        ...
        // Register as the default static instance for backward-compat API
    ...
    private async updateFrame(time: number): Promise<void> {
        ...
        const ctx = this.context.webGPUContext;
        for (let i = 0; i < views.length; i++) {
            const view = views[i];
        ...
        const cc = this.context.componentCollect;
        ...
        const globalMatrixBindGroup = this.context.globalBindGroup.modelMatrixBindGroup;
    ...
    // Delegates to the first (default) Engine3D instance created.
    // =========================================================

    /** @internal - the default instance used by the static API */
    ...
    public static get frameRate(): number { return this._default?.frameRate ?? 360; }
    ...
    public static get setting(): EngineSetting { return this._default?.setting; }
    public static set setting(v: EngineSetting) { if (this._default) this._default.setting = v; }

    /**
     * Create and initialize the default engine instance (backward-compatible static API).
     * For multi-instance usage, construct `new Engine3D()` directly.
     */
