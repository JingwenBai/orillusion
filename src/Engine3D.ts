...
import { setCurrentEngineId } from './core/EngineRegistry';

/**
 * Orillusion 3D Engine
 *
 * Create an instance and call init() to start:
 * ```ts
 * const engine = new Engine3D();
 * engine.setting.render.debug = true;
...
export class Engine3D {
    ...
    /**
     * The WebGPU context (canvas + device) for this engine instance.
     */
    ...
    /**
     * Unique identifier for this engine instance. Used to scope render resources.
     */
    ...
    /**
     * The currently executing Engine3D instance.
     * Set automatically before each render frame and during init.
     * Internal code can use `Engine3D.current` to reach the active engine.
     */
    ...
    /** Shared GPUAdapter – created once, reused by all instances. @internal */
    private static _sharedAdapter: GPUAdapter | null = null;

    /** Shared GPUDevice – created once, reused by all instances. @internal */
    private static _sharedDevice: GPUDevice | null = null;

    /** Whether core one-time resources (shaders, bind groups) have been initialised. @internal */
    private static _coreInitialized: boolean = false;

    // ─── Static compatibility getters ────────────────────────────────────────
    //  Allows existing internal rendering code (e.g. RendererJob, PassRenderers)
    //  that references `Engine3D.setting` to keep working without modification.

    /** @deprecated Access via engine instance: `engine.setting` */
    public static get setting(): EngineSetting {
        return Engine3D.current!.setting;
    }

    /** @deprecated Access via engine instance: `engine.setting = ...` */
    public static set setting(val: EngineSetting) {
        if (Engine3D.current) Engine3D.current.setting = val;
    }
    ...
    /**
     * Set engine render frameRate: 24 / 30 / 60 / 120 / 144 / 240 / 360 fps or other
     */
     ...
     * Multiple instances can be created; they share the underlying GPUDevice but
     * each has its own canvas, render targets, views and render jobs.
     *
     * @param descriptor  {@link CanvasConfig} and optional callbacks
     */
    public async init(descriptor: {
        ...
        // Merge supplied settings
        this.setting = { ...this.setting, ...descriptor.engineSetting };

        // Activate this instance so resource managers prefix correctly
        Engine3D.current = this;
        setCurrentEngineId(this.id);

        this.webGPUContext = new Context3D();
        setActiveContext(this.webGPUContext);

        if (!Engine3D._sharedDevice) {
            // First instance: initialise WASM and create a new GPUDevice
            await WasmMatrix.init(Matrix4.allocCount, this.setting.doublePrecision);
            await this.webGPUContext.init(descriptor.canvasConfig);
            Engine3D._sharedAdapter = this.webGPUContext.adapter;
            Engine3D._sharedDevice = this.webGPUContext.device;
        } else {
            // Subsequent instances: reuse the shared GPUDevice
            await this.webGPUContext.init(
                descriptor.canvasConfig,
                Engine3D._sharedDevice,
                Engine3D._sharedAdapter,
            );
        }
        ...
        // One-time shared resource initialisation (shaders, bind group layouts)
        if (!Engine3D._coreInitialized) {
            Engine3D._coreInitialized = true;
            ShaderLib.init();
            ShaderUtil.init();
    ...
    /**
     * Release all resources owned by this engine instance and stop the render loop.
     */
    public destroy() {
        this.pause();
        GBufferFrame.destroyForEngine(this.id);
        RTResourceMap.destroyForEngine(this.id);
    ...
    /**
     * Get the RendererJob for the given view.
     */
    ...
    private async _updateFrame(time: number) {
        // Activate this engine instance for the duration of this frame so that
        // all static accessors (Engine3D.setting, webGPUContext proxy, etc.)
