...
import { GPUContextState } from './gfx/renderJob/GPUContext';
...
export class Engine3D {
    ...
    /** Per-instance global bind group manager */
    public globalBindGroup: GlobalBindGroup;

    /** Per-instance shadow lights collector */
    ...
    /** Per-instance GPU render-state cache */
    public gpuContextState: GPUContextState;
    ...
    constructor() {
        ...
        this.gBufferFrames = new Map<string, GBufferFrame>();
        this.gpuContextState = new GPUContextState();
    }
    ...
    public async init(descriptor: {
        ...
        // Register this instance as the active engine BEFORE any subsystem
        // code runs so that static router methods resolve correctly during init.
        ...
        // Update the module-level webGPUContext after canvas init (size is now known)
        setActiveWebGPUContext(this.gpuContext);
    ...
    public startRenderView(view: View3D): RendererJob {
        ...
        this.views = [view];
        view.engine = this;
        // Ensure the shadow buffer is registered in this engine's collector
    ...
    private async _updateFrame(time: number) {
        // Activate this engine for all static router calls
        setActiveEngine(this);
        setActiveWebGPUContext(this.gpuContext);
    ...
    // All static members delegate to the default (first) instance so that
    // single-instance code requires zero changes.
    ...
    // ── Static convenience getters / setters ─────────────────────────────
    ...
    public static get setting(): EngineSetting { return this._defaultInstance?.setting; }
    public static set setting(v: EngineSetting) { if (this._defaultInstance) this._defaultInstance.setting = v; }
    ...
    public static get size(): number[] { return this._defaultInstance?.gpuContext?.presentationSize; }
    public static get aspect(): number { return this._defaultInstance?.gpuContext?.aspect; }
    public static get width(): number { return this._defaultInstance?.gpuContext?.windowWidth; }
    public static get height(): number { return this._defaultInstance?.gpuContext?.windowHeight; }

    // ── Static factory / lifecycle ────────────────────────────────────────

    /**
     * Create and initialize the default Engine3D instance.
     * Backward-compatible entry point for single-instance usage.
     * Returns the engine instance so callers can use it for multi-instance patterns.
     */
    ...
    public static startRenderViews(views: View3D[]): void {
        this._defaultInstance.startRenderViews(views);
    }

    public static getRenderJob(view: View3D): RendererJob {
        return this._defaultInstance?.renderJobs?.get(view);
    }
