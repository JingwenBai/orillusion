 ...
 *
 * Supports multiple independent instances — each instance owns its own
 * canvas/WebGPU context, resource manager, input system, and render targets.
 * The shared GPU device is acquired once and reused across all instances.
 *
 * Instance usage:
 * ```ts
 ...
 * ```
 *
 * Backward-compatible static usage (delegates to the first/last active instance):
 * ```ts
...
export class Engine3D {
    ...
    /**
     * Engine settings for this instance. Defaults are copied from the engine defaults.
     */
    ...
    private static _wasmMatrixInitialized: boolean = false;

    // ─── Active instance (for static backward-compat delegation) ───────

    /**
     * The currently active Engine3D instance. Static accessors delegate here.
     * Set to `this` at the start of init() and at the start of each render frame.
     */
    ...
    /** @deprecated Use instance property instead. Delegates to the active Engine3D instance. */
    ...
    /**
     * Engine setting. Delegates to the active Engine3D instance.
     * In single-engine apps this behaves identically to the old static property.
     */
    public static get setting(): EngineSetting { return Engine3D._active?.setting ?? createDefaultSetting(); }
     ...
     * Creates a WebGPU canvas context for the configured canvas. On first call across all
     * instances also requests the shared GPU adapter and device.
     */
    public async init(descriptor: {
        ...
        // Merge caller settings into this instance's setting copy
        if (descriptor.engineSetting) {
            this.setting = { ...this.setting, ...descriptor.engineSetting };
        }

        // WasmMatrix is a global WASM module — init once across all instances
        if (!Engine3D._wasmMatrixInitialized) {
            Engine3D._wasmMatrixInitialized = true;
            await WasmMatrix.init(Matrix4.allocCount, this.setting.doublePrecision);
        }

        // Make this the active instance so subsystem statics see the right context
        Engine3D._active = this;

        // Per-engine canvas context (shares the global GPU device after first init)
        ...
        GBufferFrame.setActiveMap(this._gBufferFrameMap);

        // Pre-create the reflection GBuffer for this engine
        ...
        // One-time global inits — each is guarded internally so safe to call per-engine
        ShaderLib.init();
        ShaderUtil.init();
    ...
    /**
     * Set the render view and start this engine's render loop.
     */
    ...
    /**
     * Set multiple render views and start this engine's render loop.
     */
    ...
    /** Activate this engine's per-engine context so all subsystem statics are routed correctly. */
    ...
    private async _updateFrame(time: number): Promise<void> {
        // Switch all subsystem statics to this engine before any rendering work
        this._activateContext();
    ...
    /**
     * Create a default Engine3D instance and initialize it.
     * Equivalent to `new Engine3D(); await engine.init(descriptor);` with the
     * instance stored as the active engine for static backward-compat access.
     */
    public static async init(descriptor: {
        ...
        // engine.init sets Engine3D._active = engine internally
    }

    /** @deprecated Use instance method. Delegates to the active Engine3D instance. */
