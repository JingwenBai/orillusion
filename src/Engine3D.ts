...
export type EngineInitDescriptor = {
...
function defaultSetting(): EngineSetting {
    return {
 ...
 *
 * Supports both single-instance (static API) and multi-instance usage.
 *
 ...
 * ```
 *
 * Single-instance (legacy) example:
 * ```ts
 * await Engine3D.init();
...
export class Engine3D {
    ...
    // ── active instance (for static shim delegation) ──────────────────────────

    /**
     * The engine instance whose frame is currently being rendered.
     * Set automatically by activate() at the start of each frame.
     */
    ...
    /** @internal */
    public gBufferRegistry: GBufferFrame.Registry;
    /** @internal */
    public shadowLightsCollect: ShadowLightsCollect;
    /** @internal */
    public gpuContextInstance: GPUContext;
    /** @internal */
    ...
    constructor() {
        ...
        this.gBufferRegistry = new GBufferFrame.Registry();
        this.shadowLightsCollect = new ShadowLightsCollect();
        this.gpuContextInstance = new GPUContext();
    ...
    // ── activate: make this the current engine for all subsystem static shims ──

    /**
     * Route all subsystem static calls to this engine instance.
     * Called automatically before each frame render.
     * @internal
     */
    public activate() {
        Engine3D.current = this;
        setWebGPUContext(this.gpuContext);
        ComponentCollect.current = this.componentCollect;
        Time.current = this.time;
        RTResourceMap.current = this.rtResourceMap;
        GlobalBindGroup.current = this.globalBindGroup;
        GBufferFrame.current = this.gBufferRegistry;
        ShadowLightsCollect.current = this.shadowLightsCollect;
        GPUContext.current = this.gpuContextInstance;
        ShaderUtil.current = this.shaderUtil;
    }

    // ── instance init ─────────────────────────────────────────────────────────

    /**
     * Create and initialise this engine instance.
     * @param descriptor  {@link EngineInitDescriptor}
     */
    public async init(descriptor: EngineInitDescriptor = {}) {
        ...
        // Activate before any subsystem init so they all use this engine's context.
        this.activate();
    ...
    private async render(time: number) {
        ...
        // Activate this engine so all static-shim subsystem calls are routed here.
        this.activate();
    ...
    // ── static shims — delegate to Engine3D.current ───────────────────────────
    // All existing single-instance call sites keep working without modification.

    /**
     * Create webgpu 3d engine (static / single-instance API).
     * Creates and activates a new Engine3D instance as Engine3D.current.
     */
    public static async init(descriptor: EngineInitDescriptor = {}) {
        const engine = new Engine3D();
        // Transfer any pre-configured setting from the static field if it was set.
        if ((Engine3D as any)._staticSetting) {
            engine.setting = { ...engine.setting, ...(Engine3D as any)._staticSetting };
        }
    ...
    /**
     * engine setting (static accessor — delegates to Engine3D.current.setting)
     */
    public static get setting(): EngineSetting {
        if (!Engine3D.current) {
            // Before init(), return a temporary setting object that init() will pick up.
            if (!(Engine3D as any)._staticSetting) {
                (Engine3D as any)._staticSetting = defaultSetting();
            }
            return (Engine3D as any)._staticSetting;
        }
        return Engine3D.current.setting;
    }
    public static set setting(v: EngineSetting) {
        if (!Engine3D.current) {
            (Engine3D as any)._staticSetting = v;
        } else {
