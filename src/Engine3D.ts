...
export class Engine3D {
    ...
    /** @internal */
    public static get renderJobs_static(): Map<View3D, RendererJob> {
        return Engine3D._defaultInstance?.renderJobs;
    }

    /**
     * Static init — creates and stores the default Engine3D instance.
    ...
    /**
     * Per-instance component lifecycle manager.
     * Components in this engine's scenes register here for per-frame updates.
     */
    public componentCollect: ComponentCollect;

    /**
     * Engine settings (per-instance, so different engines can have different configs).
     */
     ...
     * Call `await engine.init(...)` to set it up before use.
     *
     * The first Engine3D instance created automatically becomes the static
     * default, so existing code using `Engine3D.setting.*` / `Engine3D.res`
     * etc. continues to work without changes.
     */
    constructor() {
        this.componentCollect = new ComponentCollect();
        // Register as default instance so static accessors work for single-instance code
    ...
    /**
     * Initialize this engine instance (WebGPU context, shaders, resources, etc.).
     * The first engine to call init() acquires the WebGPU device; subsequent engines
     * reuse the shared device.
     */
    public async init(descriptor: {
        ...
        // WasmMatrix and the WebGPU device are initialized only once (shared).
    ...
    /**
     * Register a view and start the render loop for this engine instance.
     */
    public startRenderView(view: View3D): RendererJob {
        this.renderJobs ||= new Map<View3D, RendererJob>();
        this.views = [view];
        view.engine = this;
        ShadowLightsCollect.createBuffer(view);
    ...
    public startRenderViews(views: View3D[]): void {
        ...
        for (let i = 0; i < views.length; i++) {
            views[i].engine = this;
            ShadowLightsCollect.createBuffer(views[i]);
