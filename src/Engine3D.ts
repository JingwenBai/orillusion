 ...
 *
 * Supports multiple simultaneous instances. Each instance owns its own
 * GPU context, resource managers, and scene state. Before each render
 * frame the instance activates its managers so that all sub-systems
 * reference the correct per-engine state.
 *
 * @group engine3D
 */
export class Engine3D {
    ...
    /**
     * The engine instance that is currently active (rendering or initialising).
     * Static accessors below delegate to this instance so that existing code
     * using `Engine3D.setting`, `Engine3D.res`, etc. continues to work.
     */
    private static _active: Engine3D;

    // ---- Static delegate getters (mirror the instance API) ----

    /** @deprecated Use an Engine3D instance instead of the static accessor. */
    ...
    /** @deprecated Use an Engine3D instance instead of the static accessor. */
    public static get frameRate(): number { return this._active?.frameRate; }
    /** @deprecated Use an Engine3D instance instead of the static accessor. */
    public static set frameRate(v: number) { if (this._active) this._active.frameRate = v; }

    // ---- Static method delegates ----

    /** @deprecated Use an Engine3D instance instead of the static method. */
    ...
    /** @deprecated Use an Engine3D instance instead of the static method. */
    public static startRenderView(view: View3D): RendererJob { return this._active?.startRenderView(view); }
    /** @deprecated Use an Engine3D instance instead of the static method. */
    public static startRenderViews(views: View3D[]) { this._active?.startRenderViews(views); }
    /** @deprecated Use an Engine3D instance instead of the static method. */
    public static getRenderJob(view: View3D): RendererJob { return this._active?.getRenderJob(view); }
    /** @deprecated Use an Engine3D instance instead of the static method. */
    public static pause() { this._active?.pause(); }
    /** @deprecated Use an Engine3D instance instead of the static method. */
    public static resume() { this._active?.resume(); }
    ...
    /**
     * Activate this engine's managers so that all sub-systems reference its state.
     * Called automatically before each render frame and during init.
     * @internal
     */
    public activateContext() {
        ...
        EntityCollect.activate(this._entityCollect);
        ComponentCollect.activate(this._componentCollect);
        RTResourceMap.activate(this._rtResourceMap);
        ShaderUtil.activate(this._shaderUtil);
        ShadowLightsCollect.activate(this._shadowLightsCollect);
        PipelinePool.activate(this._pipelinePool);
        GBufferFrame.setActiveMap(this._gBufferMap);
    }

    /**
     * Create and initialise a WebGPU 3D engine instance.
    ...
    public async init(descriptor: { canvasConfig?: CanvasConfig; beforeRender?: Function; renderLoop?: Function; lateRender?: Function, engineSetting?: EngineSetting } = {}) {
        ...
        // Create per-engine manager instances
        ...
        // Create per-engine GPU context
        this.context = new Context3D();

        // Activate this engine's context so subsequent init calls use correct state
        this.activateContext();

        await WasmMatrix.init(Matrix4.allocCount, this.setting.doublePrecision);

        await this.context.init(descriptor.canvasConfig);
        // Re-activate after context init (presentationSize is now known)
        this.activateContext();
    ...
    private async render(time: number) {
        ...
        // Switch all global statics to this engine before frame work
        this.activateContext();
