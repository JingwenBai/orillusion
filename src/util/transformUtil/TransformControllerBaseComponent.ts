...
export class TransformControllerBaseComponent extends ComponentBase {
    ...
    public onMouseDown(e: PointerEvent3D): void {
        ...
        let pos = camera.screenPointToWorld(Engine3D.current.inputSystem.mouseX, Engine3D.current.inputSystem.mouseY, screenPoint.z);
        this.beginPoint.copyFrom(pos);
        this.beginMousePos.x = Engine3D.current.inputSystem.mouseX;
        this.beginMousePos.y = Engine3D.current.inputSystem.mouseY;
    }
    ...
    public onMouseMove(e: PointerEvent3D): void {
        if (this.currentAxis == TransformAxisEnum.NONE) {
            ...
            Vector3.HELP_0.set(Engine3D.current.inputSystem.mouseX, Engine3D.current.inputSystem.mouseY, 0);
